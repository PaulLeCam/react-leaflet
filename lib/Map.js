...
var Map = function (_MapComponent) {
  ...
  function Map(props, context) {
    ...
    _this.handleMapStateChange = function () {
      var onMapStateChange = _this.props.onMapStateChange;

      if (onMapStateChange) {
        var handleEvent = function handleEvent() {
          onMapStateChange({
            bounds: _this.leafletElement.getBounds(),
            center: _this.leafletElement.getCenter(),
            size: _this.leafletElement.getSize(),
            zoom: _this.leafletElement.getZoom()
          });
        };
        _this.leafletElement.on('moveend', handleEvent);
        _this.leafletElement.on('resize', handleEvent);
        _this.leafletElement.on('zoomend', handleEvent);
      }
    };
...
Map.propTypes = {
  ...
  onMapStateChange: _react.PropTypes.func
};
Map.defaultProps = {
  animate: false,
  useFlyTo: false
